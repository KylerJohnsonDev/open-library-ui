  ...
  template: `
    <div class="min-h-screen bg-background text-foreground p-4 md:p-8 font-serif">
      <header class="max-w-6xl mx-auto mb-12 text-center">
        ...
        <p class="text-muted-foreground text-lg italic">Explore the boundless archives of Open Library</p>
      ...
      <main class="max-w-6xl mx-auto">
        @if (store.searchIsError()) {
          <div class="p-4 mb-6 rounded-lg bg-destructive/10 text-destructive border border-destructive/20 text-center">
        ...
        @if (store.searchIsPending() && store.books().length === 0) {
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            @for (book of store.books(); track book.key) {
              <z-card class="group flex flex-col h-full overflow-hidden border-border/50 hover:border-primary/50 transition-all hover:shadow-xl bg-card/50 backdrop-blur-sm">
                <div class="relative h-64 overflow-hidden bg-muted flex items-center justify-center">
                  @if (book.cover_i) {
                    <img 
                  ...
                  @if (book.subject) {
                    <div class="flex flex-wrap gap-1 mt-auto">
                      @for (sub of book.subject.slice(0, 3); track sub) {
                        <z-badge variant="outline" class="text-[10px] px-2 py-0 border-primary/20 text-primary/70">
                          {{ sub }}
                        </z-badge>
                      }
                    </div>
          ...
          <div class="text-center py-20 bg-card/30 rounded-3xl border-2 border-dashed border-border/50">
           ...
           <div class="text-center py-20">
             ...
             <p class="text-muted-foreground max-w-md mx-auto">
               Enter a title, author, or secret keyword to uncover hidden treasures from the Open Library collection.
             </p>
           </div>
        }
      </main>

      <footer class="mt-20 py-8 border-t border-border/50 text-center text-sm text-muted-foreground font-sans">
